...
class Generator:
    ...
    def corridor_between_points(self, x1, y1, x2, y2, join_type='either'):
        if x1 == x2 and y1 == y2 or x1 == x2 or y1 == y2:
            ...
            # TODO better corridor placement, remove corridor aligning with walls
            join = None
            ...
            elif join_type is 'either' and {self.width - 1, self.width - 2}.intersection({x1, x2}) or {
    ...
    def join_rooms(self, room_1, room_2, join_type='either'):
        ...
        x1 = sorted_room[0][0]+1
        y1 = sorted_room[0][1]+1
        w1 = sorted_room[0][2]-1
        h1 = sorted_room[0][3]-1
        x1_2 = x1 + w1 - 1
        y1_2 = y1 + h1 - 1

        x2 = sorted_room[1][0]+1
        y2 = sorted_room[1][1]+1
        w2 = sorted_room[1][2]-1
        h2 = sorted_room[1][3]-1
        x2_2 = x2 + w2 - 1
        y2_2 = y2 + h2 - 1
...
def Create_FloorMap(size):
    ...
    if size == 'S':
        ...
        size = 48
        MaxRoom, MinSize, MaxSize = 8, 8, 14

    elif size == 'L':
        size = 64
        MaxRoom, MinSize, MaxSize = 12, 12, 18

    elif size == 'H':
        size = 80
        MaxRoom, MinSize, MaxSize = 10, 10, 30
    ...
    Start = gen.RandLocal()
    rooms = gen.room_list
    #print(len(floor_array), len(floor_array[0]))
    #t2 = time.time()
    #print(t2- t1)
